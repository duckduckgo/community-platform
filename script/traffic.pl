...
system( qq(sudo /usr/bin/s3cmd -c /root/.s3cfg ls s3://ddg-statistics/ | sort -k1 -r | head -n1 | awk {'print $4'} | xargs /usr/bin/s3cmd -c /root/.s3cfg get && gzip -df statistics_*.gz) );
