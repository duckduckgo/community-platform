...
sub broken_bang {[
  ...
  { name => "issue", description => "The issue is that", type => "text", placeholder => "e.g. the site returns an error, the site no longer exists", icon => "bug" },
  { name => 'email', description => "Your email (not required)", type => "email", icon => "inbox", optional => 1},
  { name => 'submit', description => "Report broken !bang", cssclass => "fb-step--submit", icon => "mail" }
]}
