...
sub bbcode {
	my ( $self ) = @_;
	my $targetblank = ($self->ddgc->current_user && $self->ddgc->current_user->forum_links_same_window) ? 'target="_top"' : 'target="_blank"';
	my %base = (
	...
	for my $site (keys %base) {
		...
		$tags{$key} = {
			...
			code => sub {
				...
				return '<a ' . $targetblank . ' href="'.$url.'">'.$attr.' @ '.$site.'</a>';
			},
		};
	}
	$tags{img} = {
		...
		code => sub {
			...
			if ($uri->can('host')) {
				...
				my $desc = $content;
				'<a rel="nofollow" '. $targetblank .' href="'.$proxy_url.'">'.
	...
	my $url_finder_format = '<a ' .  $targetblank . ' href="%s" rel="nofollow">%s</a>';
...
sub url_parse {
	...
	if ($uri->can('host')) {
		...
		'<a ' . $targetblank . ' href="'.$url.'" rel="nofollow">'.$desc.'</a>'
	} else {
