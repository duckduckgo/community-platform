...
** the modal trigger elements lost their "data-reveal-id" info that tells JS which modal to open.  also, there is a javascript error in jquery.reveal - could be a compatibility thing.  I think we should go back to my modified version. -D
...
* green/red on hover, and if selected green/red permanently?
